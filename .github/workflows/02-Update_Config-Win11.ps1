...
<#
.SYNOPSIS
  Checks and sets Windows Update (WU) UX settings, Windows Security settings, and checks for Microsoft Store updates.
.NOTES
  UX keys can be overridden by policy under HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate
  Combined script that configures Windows Update, Windows Security status, and Microsoft Store updates
#>

# Load UI Automation assemblies for MS Store update checking
...
# --- Microsoft Store Update Functions (from Check_MSstore-Updates.ps1) ---

function Check-MSStoreUpdates {
    Write-SectionHeader "Microsoft Store Updates" "🏪"

    Write-Host "`n  Opening Microsoft Store Downloads page..." -ForegroundColor Cyan
